...
addMotifs <- function(sco,motifs=7){
  ...
  if(!is.numeric(motifs)|!length(motifs)==1) stop("Motifs needs to be a number representing a k-mer from 5 to 8")
    ...
    if(file.exists(file)) load(file) else stop(paste0("Please prepare motif models for ",motifs,"-mers."))
  ...
  # check if seqXmot calculations were already done (True for 5,6,7 mers)
    ...
    if(!file.exists(file)) stop(paste0("Please prepare sequence specific probabilities for ",motifs,"-mers in species ",sco$runparameters$species,"."))
  ...
  # count number of motifs in sequences?
    ...
    ################################# # in case motifs are standard k-mers, re-use if possible (5,6,7mers)
      ...
      if(!file.exists(file)) stop(paste0("Please prepare motif counts for ",motifs,"-mers in species ",sco$runparameters$species,"."))
      cat("Premade. Loading...\n")
      counts <- readRDS(file)
